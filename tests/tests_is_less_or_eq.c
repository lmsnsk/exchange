...
START_TEST(s21_test_is_less_or_equal_5) {
  s21_decimal a = {{S21_MAX_UINT, S21_MAX_UINT - 1, 0, 0}};
...
START_TEST(s21_test_is_less_or_equal_8) {
  ...
  s21_decimal b = {{0, S21_MAX_UINT, S21_MAX_UINT, 0}};
  ck_assert_int_eq(s21_is_less_or_equal(a, b), 1);
}
END_TEST
...
Suite *suite_is_less_or_equal(void) {
  Suite *s;
  TCase *tc;
  s = suite_create("\033[46m---IS LESS OR EQ TESTS---\033[0m");
  tc = tcase_create("is_less_or_equal_test");

  tcase_add_test(tc, s21_less_or_equal);
